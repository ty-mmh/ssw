...
const SecureChatApp = () => {
  ...
  const handleEnterSpace = useCallback(async () => {
    if (!passphrase.trim()) { setError('合言葉を入力してください。'); return; }
    setIsLoading(true); setError(''); setEncryptionStatus('initializing');
    try {
  ...
  const handleSendMessage = useCallback(async () => {
    ...
    setIsLoading(true);
    try {
      const sentMessage = await window.API.sendMessageFriendly(currentSpace.id, message);
      setMessages(prev => [...prev, sentMessage]);
      setMessage('');
      if(socket) {
        const payloadForBroadcast = { ...sentMessage, timestamp: sentMessage.timestamp.toISOString() };
  ...
  useEffect(() => {
    ...
    newSocket.on('message-received', async (data) => {
      ...
      if (data.message && data.message.space_id === currentSpace.id) {
        const activeSessionIds = window.SessionManager.getActiveSessionsForSpace(currentSpace.id);
        ...
        if (receivedMessage.encrypted) {
          try {
            ...
            if (!payloadToDecrypt) { throw new Error("暗号化ペイロードが見つかりません。"); }
            
            const decryptedText = await window.Crypto.decryptMessageWithFallback(
                payloadToDecrypt,
                currentSpace.id, 
                activeSessionIds
            );
            ...
            console.warn(`[リアルタイム] メッセージ[${receivedMessage.id}]の復号化に失敗:`, e.message);
        ...
        setMessages(prev => {
            ...
            if (messageExists) {
                return prev.map(msg => msg.id === receivedMessage.id ? receivedMessage : msg);
            } else {
    ...
    newSocket.on('space-joined-successfully', (data) => {
        if (data.spaceId === currentSpace.id && window.SessionManager) {
            console.log('[App] 空間参加成功。サーバーからセッション状態を同期します。', data);
  ...
  useEffect(() => {
    ...
    return () => document.removeEventListener('encryption-level-changed', updateStatus);
  }, [currentSpace]);
  ...
  useEffect(() => {
    const handleSessionStateChange = (event) => {
        if (currentSpace && event.detail.spaceId === currentSpace.id) {
            console.log('[App] セッション状態の変更を検知。キー交換を再トリガーします。', event.detail);
            ...
            if (window.KeyExchangeManager) {
                ...
                setTimeout(() => {
                    console.log("[App] 変更に応じて、自身の公開鍵を再アナウンスします。");
                    window.KeyExchangeManager.announcePublicKey();
                }, Math.random() * 500 + 200); // 200-700msのランダムな遅延で通知の衝突を避ける
            }
        }
    };
    // [修正] イベント名を `session-state-changed` に変更
    document.addEventListener('session-state-changed', handleSessionStateChange);
    return () => document.removeEventListener('session-state-changed', handleSessionStateChange);
...
document.addEventListener('DOMContentLoaded', () => {
    ReactDOM.createRoot(document.getElementById('root')).render(React.createElement(SecureChatApp));
