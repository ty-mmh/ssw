...
window.IntegratedChatUI = ({
  ...
  return React.createElement(
    'div', { className: 'h-screen w-screen bg-gradient-to-br from-gray-900 via-stone-950 to-black text-gray-200 flex flex-col overflow-hidden' },
    // Header (変更なし)
    React.createElement('header', { className: 'glass-pane z-10 flex-shrink-0' },
      React.createElement('div', { className: 'max-w-5xl mx-auto p-3 sm:p-4 flex justify-between items-center' },
        ...
        React.createElement('div', { className: 'flex items-center gap-2 sm:gap-4' },
          ...
          React.createElement('button', { onClick: onLeaveSpace, className: 'bg-danger/80 hover:bg-danger px-3 py-2 rounded-lg text-sm transition' }, '退室')
        )
      )
    ),
    // [修正] Message List のレイアウトを通常の top-to-bottom に戻す
    React.createElement('main', { ref: chatContainerRef, className: 'flex-1 overflow-y-auto p-4' },
      React.createElement('div', { className: 'max-w-5xl mx-auto space-y-4' },
        sortedMessages.length > 0 ?
          // [修正] mapの対象を昇順ソート済みの配列に変更
          sortedMessages.map(msg => React.createElement(window.EnhancedMessageDisplay, { key: msg.id, message: msg })) :
          React.createElement('div', { className: 'text-center text-gray-500 pt-20 animate-fade-in' },
    ...
    // Footer
    React.createElement('footer', { className: 'glass-pane sticky bottom-0 z-10' },
      React.createElement('div', { className: 'max-w-5xl mx-auto p-2 sm:p-4' },
        React.createElement('div', { className: 'flex gap-3 items-end' },
          ...
          React.createElement('button', {
              onClick: onSendMessage, disabled: isLoading || !message.trim(),
              className: 'h-11 bg-primary hover:bg-blue-600 disabled:bg-gray-600 px-5 rounded-lg font-medium transition-all duration-200 flex items-center justify-center whitespace-nowrap'
            },
            isLoading ? React.createElement('div', {className: 'w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin'}) : '送信'
          )
        )
      )
    )
