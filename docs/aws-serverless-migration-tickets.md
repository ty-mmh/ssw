...
- 既存 `secure_chat.db` の本番データ移行は Ticket 09 まで留保する。
...
## Ticket 09: Data Migration Plan And Scripts
...
- `secure_chat.db` から Spaces/Messages を読み出す移行スクリプトを用意する。
- 平文 `passphrase` を `passphraseHash` へ変換して移行する。
- `public/uploads` の暗号化済みファイルをS3へコピーし、message参照を `s3Key` へ変換する。
- dry-run と件数サマリを出す。

### Reservations

- 実験環境では新規DBで始めてもよい。その場合、このチケットは後回しにできる。
- 既存DBにサンプルデータや不要データが混じる可能性があるため、本番実行前に対象DBの確認が必要。
- 外部S3への実アップロードはユーザーが明示的に許可したタイミングで行う。
- 失敗時のロールバックは、移行先テーブルとS3 prefixを環境ごとに分けることで対応する。

### Done

- dry-runで移行件数と変換内容が確認できる。
- 実行時にDynamoDB/S3へ移行できる。
- 既存メッセージの復号に必要な情報が失われない。
