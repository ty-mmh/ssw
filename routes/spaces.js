...
module.exports = (db) => {
  ...
  router.post('/enter', (req, res) => {
    ...
    try {
      ...
      if (space) {
        db.prepare(
          "UPDATE spaces SET last_activity_at = datetime('now') WHERE id = ?",
        ).run(space.id)
        res.json({ success: true, space })
      } else {
        res
          .status(404)
          .json({ success: false, error: 'その合言葉の空間は存在しません。' })
      }
    } catch (error) {
  ...
  router.post('/create', (req, res) => {
    ...
    try {
      // 既存チェック
      const existingSpace = db
        .prepare('SELECT id FROM spaces WHERE passphrase = ?')
        .get(validation.passphrase)
      if (existingSpace) {
        return res
          .status(409)
          .json({ success: false, error: 'その合言葉は既に使用されています。' })
      }
