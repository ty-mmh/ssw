...
io.on('connection', (socket) => {
  ...
  socket.on('new-message', (data) => {
    ...
    console.log(`--------------------------------`);
    console.log(`💬 [診断] 'new-message' 受信 (from: ${socket.id.substring(0,5)}...)`);
    console.log(`   - 空間ID: ${roomName}`);
    console.log(`   - 現在のルーム参加者: [${clientsInRoom.join(', ')}]`);
    console.log(`   - 送信者以外の対象者数: ${clientsInRoom.filter(id => id !== socket.id).length}人`);
  ...
  socket.on('public-key-announcement', (data) => {
      console.log(`🔑 [Socket.IO] 公開鍵通知 from ${data.sessionId.substring(0,8)} in ${data.spaceId}`);
  ...
  socket.on('disconnect', () => {
    ...
    activeSessions.forEach((sessions, spaceId) => {
      if (sessions.has(socket.id)) {
        ...
        io.to(spaceId).emit('session-left', { sessionId: socket.id, spaceId, timestamp: new Date().toISOString() });
        io.to(spaceId).emit('session-count-updated', { spaceId, sessionCount, encryptionLevel: sessionCount > 1 ? 'hybrid' : 'deterministic' });
        if (sessionCount === 0) {
...
const cleanupExpiredMessages = () => {
  try {
    ...
    if (result.changes > 0) {
      console.log(`🧹 [Auto-Cleanup] 期限切れメッセージを ${result.changes} 件クリーンアップしました。`);
    }
  } catch (error) {
    console.error('❌ [Auto-Cleanup] メッセージのクリーンアップ中にエラーが発生しました:', error);
  }
};
...
console.log(`🧹 期限切れメッセージの自動クリーンアップが有効になりました (実行間隔: ${CLEANUP_INTERVAL_MS / (1000 * 60)}分)`);
